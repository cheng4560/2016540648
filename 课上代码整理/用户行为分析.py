...
user_purchase_retention = pd.merge(left=data_t, right=orderdt_min.reset_index(), how='inner', on='用户ID', suffixes=('', 'min'))
...
#每一次消费时间与第一次消费时间间隔
user_purchase_retention['order_date_diff'] = user_purchase_retention['购买日期']-user_purchase_retention['购买日期min']
#将timedelta转换为数值型
